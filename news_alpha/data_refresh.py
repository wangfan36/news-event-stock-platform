...
def refresh_news_data(rss_sources_text: str = "") -> dict[str, Any]:
    ...
    if not clustered:
        status = get_news_data_status()
        status.update(
            {
                "refresh_ok": False,
                "refresh_error": "RSS 源未返回可解析新闻，已保留上一次有效缓存。",
            }
        )
        return status
    ...
    status = get_news_data_status()
    status.update({"refresh_ok": True, "refresh_error": ""})
    return status
...
def get_stock_data_status() -> dict[str, Any]:
    config = load_config()
    primary_as_of = ""
    if config.parquet_path is not None and config.parquet_path.exists():
        try:
    ...
    note = f"A股主数据 {primary_as_of}" if primary_as_of else "未配置本地 A 股行情数据"
    if override_as_of:
