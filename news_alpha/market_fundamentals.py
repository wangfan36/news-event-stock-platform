...
def _build_price_snapshot(instrument: str, as_of_ts: pd.Timestamp) -> dict[str, Any]:
    ...
    if config.parquet_path is None or not config.parquet_path.exists():
        return {"status": "missing", "reason": "local price parquet is not configured"}
    try:
        df = pd.read_parquet(
            config.parquet_path,
...
def _build_valuation_snapshot(ts_code: str, as_of_ts: pd.Timestamp) -> dict[str, Any]:
    config = load_app_config()
    if config.parquet_path is None:
        return {"status": "missing", "reason": "local valuation data is not configured"}
...
def _build_fundamental_snapshot(ts_code: str, symbol: str, as_of_ts: pd.Timestamp) -> dict[str, Any]:
    ...
    if config.parquet_path is None:
        ...
        return fallback or {"status": "missing", "reason": "local fundamental data is not configured"}
...
def _load_a_share_abstract_payload(symbol: str) -> dict[str, Any] | None:
    if os.getenv("NEWS_ALPHA_ENABLE_NETWORK_FUNDAMENTALS", "0") != "1":
        return None
    try:
