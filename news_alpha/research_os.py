...
from .symbols import load_universe_symbols, normalize_symbol
...
def _runtime_candidate_universe() -> set[str]:
    configured = set(load_universe_symbols(get_app_config().universe_path))
    if configured:
        return configured
    catalog_symbols = set(COMPANY_CATALOG)
    catalog_symbols.update(A_SHARE_NAME_MAP)
