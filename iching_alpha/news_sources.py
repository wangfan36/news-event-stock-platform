...
def fetch_rss_source(source: dict[str, str], timeout: float = 5.0, limit: int = 8) -> list[dict[str, Any]]:
    ...
    items: list[dict[str, Any]] = []
    for raw_item in channel.findall("item")[:limit]:
        title = _node_text(raw_item, "title")
        description = _clean_description(_node_text(raw_item, "description"))
        source_url = _node_text(raw_item, "link")
        published_offset_hours = 6
        items.append(
            {
                "source_id": f"{source['source_id']}::{_slugify(title)}",
                "headline": title,
